...
def overall_fitness(results_per_task: Dict[str, tuple]) -> float:

    mean_fitness = 0.0
    for task_name, results in results_per_task.items():
        fi = fitness(np.array(results).reshape(1, -1))[0]
        mean_fitness += fi
    mean_fitness /= len(results_per_task)
...
class Metric:
    def __init__(self) -> None:
        self.p: np.ndarray = np.array([])  # (nc, )
        self.r: np.ndarray = np.array([])  # (nc, )
        self.f1: np.ndarray = np.array([])  # (nc, )
        self.all_ap: np.ndarray = np.array([])  # (nc, 10)
        self.ap_class_index: np.ndarray = np.array([])  # (nc, )
...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir="", names=(), f_name="confusion_matrix.png"):
        ...
        fig.savefig(Path(save_dir) / f_name, dpi=250)
        plt.close(fig)
